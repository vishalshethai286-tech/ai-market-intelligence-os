...
import { getMissingContactCoverageExportRows, MISSING_CONTACT_COVERAGE_EXPORT_COLUMNS } from "@/lib/export/service";
...
export async function GET() {
  ...
  const rows = await getMissingContactCoverageExportRows(active.workspace.id);
  return new NextResponse(toCsv(rows, MISSING_CONTACT_COVERAGE_EXPORT_COLUMNS), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="missing-contact-coverage.csv"',
    },
  });
}
