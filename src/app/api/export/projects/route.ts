...
import { getProjectsExportRows, PROJECT_EXPORT_COLUMNS } from "@/lib/export/service";
...
export async function GET() {
  ...
  const rows = await getProjectsExportRows(active.workspace.id);
  return new NextResponse(toCsv(rows, PROJECT_EXPORT_COLUMNS), {
    headers: {
