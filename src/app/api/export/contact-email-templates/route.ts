...
import { getContactEmailTemplatesExportRows, CONTACT_EMAIL_TEMPLATE_EXPORT_COLUMNS } from "@/lib/export/service";
...
export async function GET() {
  ...
  const rows = await getContactEmailTemplatesExportRows(active.workspace.id);
  return new NextResponse(toCsv(rows, CONTACT_EMAIL_TEMPLATE_EXPORT_COLUMNS), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="contact-email-templates.csv"',
    },
  });
}
